...
`HOST`, `ALU AND`, `ALU OR`, `ALU XOR`, `ALU ADD`, `ALU SUB`, `ALU SHL`, `ALU SHR`, `JMP`, `JZ`, `JNZ`, `JPIN`, `OUTA`, and `HALT` are
...
| `7` | `ALU fn,imm8` | `[11:8]`, `[7:0]` | Transform accumulator; functions `0`–`3` are AND, OR, XOR, and wrapping ADD immediate, functions `4`/`5` are logical shift-left/right by `[2:0]`, and function `6` is wrapping SUB immediate |
...
`HOST`, `ALU AND`, `ALU OR`, `ALU ADD`, `ALU SUB`, `ALU SHL`, `ALU SHR`, `JMP`, `JZ`, `JNZ`, `JPIN`, and `OUTA` are implemented in draft
