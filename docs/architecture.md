...
`HOST`, `ALU AND`, `ALU OR`, `ALU XOR`, `ALU SHR`, `JMP`, `JZ`, `JNZ`, `OUTA`, and `HALT` are
implemented and tested. Firmware can sample an external or host byte, transform
it with AND-immediate, OR-immediate, XOR-immediate, or logical shift-right, and later drive the retained value
...
| `7` | `ALU fn,imm8` | `[11:8]`, `[7:0]` | Transform accumulator; function `0` is AND-immediate, function `1` is OR-immediate, function `2` is XOR-immediate, and function `5` is logical shift-right by `[2:0]` |
...
`HOST`, `ALU AND`, `ALU OR`, `ALU SHR`, `JMP`, `JZ`, `JNZ`, and `OUTA` are implemented in draft
