...
and 33 words when waits are needed, plus one for an optional XOR transform; host
input adds a leading `NOP` so `run` can be pulsed before all eight host-input bits
carry data. Runtime UART firmware can also keep independently selected background
pins enabled during the frame.
