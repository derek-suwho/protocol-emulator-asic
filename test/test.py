...
@cocotb.test()
async def test_alu_rotate_left_immediate_preserves_shifted_out_bits(dut):
    """ALU ROL-immediate must wrap shifted-out bits back into the accumulator."""
    cocotb.start_soon(Clock(dut.clk, CLOCK_PERIOD_US, unit="us").start())
    await reset_dut(dut)

    # LDI 0x81; ALU ROL,1; OUTA; HALT. Rotation produces 0x03, unlike SHL's 0x02.
    for instruction in (0x5081, 0x7801, 0xC000, 0xF000):
        await load_instruction_lsb_first(dut, instruction)
...
@cocotb.test()
async def test_alu_population_count_counts_set_bits(dut):
    """ALU POPCNT must replace the accumulator with its number of set bits."""
    cocotb.start_soon(Clock(dut.clk, CLOCK_PERIOD_US, unit="us").start())
    await reset_dut(dut)

    # LDI 0xB5; ALU POPCNT; OUTA; HALT. 0xB5 contains five set bits.
    for instruction in (0x50B5, 0x7E00, 0xC000, 0xF000):
        await load_instruction_lsb_first(dut, instruction)
...
@cocotb.test()
async def test_outbit_copies_accumulator_lsb_to_selected_output(dut):
    """OUTBIT must update one selected output without disturbing other pins."""
    cocotb.start_soon(Clock(dut.clk, CLOCK_PERIOD_US, unit="us").start())
    await reset_dut(dut)

    # OUT 0xA0; LDI 1; OUTBIT pin 2; HALT. Only pin 2 should be added.
    for instruction in (0x10A0, 0x5001, 0xD002, 0xF000):
        await load_instruction_lsb_first(dut, instruction)
...
@cocotb.test()
async def test_oea_drives_output_enable_from_accumulator(dut):
    """OEA must copy the runtime accumulator value to output-enable controls."""
    cocotb.start_soon(Clock(dut.clk, CLOCK_PERIOD_US, unit="us").start())
    await reset_dut(dut)

    # LDI 0xA5; OEA; HALT.
    for instruction in (0x50A5, 0xE000, 0xF000):
        await load_instruction_lsb_first(dut, instruction)
